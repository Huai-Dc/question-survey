<template>
  <div class="survey-content">
    ...
    <div class="question-tips"><span style="font-size: 14px; color: #52a9ff;">{{currentIndex}}</span> /
      {{questions.length}} <span v-if="currentQuestion">{{getAnswerType(currentQuestion.subAnswerType)}}</span></div>
    ...
    <div v-if="currentQuestion" class="question-area">
      ...
      <template v-if="currentQuestion.subAnswerType == '2'" v-for="(item, index) in currentQuestion.items">
        <van-checkbox v-model="item.checked" @change="checkSub(item.checked, item)">{{item.itemName}}</van-checkbox>
        <div v-if="item.childSubject && item.childSubject.childItems && item.childSubject.childItems.length>0">
          ...
          <van-radio-group v-show="item.checked" v-if="item.childSubject && item.childSubject.subAnswerType == '1'" v-model="item.childSubject.value">
          ...
          <template v-if="item.childSubject && item.childSubject.subAnswerType == '2'" v-for="(childItem, index) in item.childSubject.childItems">
            <template>
              <van-checkbox v-show="item.checked" style="margin-left: 20px" v-model="childItem.checked" @change="checkSub(childItem.checked, item)">{{childItem.itemName}}</van-checkbox>
              <van-field
    ...
    <div class="handler-area">
      <van-button type="info" class="handle-button" :disabled="currentIndex <= 1" @click="setCurrentQuestion(+currentIndex-1)">上一题</van-button>
      <van-button type="info" class="handle-button" v-if="currentIndex < questions.length" @click="setCurrentQuestion(+currentIndex+1)">下一题</van-button>

      <van-button type="info" class="handle-button" v-if="currentIndex == questions.length" @click="finish">完成</van-button>
    </div>
